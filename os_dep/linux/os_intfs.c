...
u8 rtw_init_default_value(_adapter *padapter)
{
	...
	/* for debug purpose */
	init_waitqueue_head(&padapter->rate_ctl_event_wq);
	spin_lock_init(&padapter->rate_ctl_event_lock);
	padapter->rate_ctl_event_q_head = 0;
	padapter->rate_ctl_event_q_tail = 0;
	padapter->fix_rate = 0xFF;
	padapter->fix_bw = 0xFF;
	padapter->rate_ctl_pending_drop = 0;
	padapter->rate_ctl_pending_macid = 0xFF;
	padapter->rate_ctl_pending_from_rate = 0xFF;
	padapter->rate_ctl_pending_to_rate = 0xFF;
	padapter->rate_ctl_pending_from_bw = 0xFF;
	padapter->rate_ctl_pending_to_bw = 0xFF;
	padapter->rate_ctl_pending_delay_ms = 0;
	padapter->data_fb = 0;
	padapter->fix_bw = 0xFF;
...
int _netdev_open(struct net_device *pnetdev)
{
	...
	if (!rtw_is_hw_init_completed(padapter)) { // ips 
...

		{
			...
			_set_timer(&adapter_to_dvobj(padapter)->dynamic_chk_timer, 200);
